...
import { SOURCE_X, SOURCE_Y } from "./sand_part2";
...
export class Grid {
  ...
  blockedNozzle(): boolean {
    return this.grid[SOURCE_Y][SOURCE_X] === "o";
  }
  ...
  drawGround() {
    this.drawLine(this.minX, this.maxY, this.maxX, this.maxY);
  }

  // Pass in an offset to print lines from the bottom
  print(offset = 0) {
    let lines = "";
    for (
      let yy = offset ? this.maxY - offset : this.minY;
      yy <= this.maxY;
      yy++
    ) {
      let line = "";
