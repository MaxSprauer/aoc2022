...
class Grid {
  ...
  step(pos: Point): Point | null {
    ...
    if (distances.values()[0] === Infinity) {
      return null;
    }

    for (const d of distances.entries()) {
      if (unvisited.has(d[0])) {
        const [x, y] = d[0].split(",");
...
for (let yy = 0; yy < grid.height; yy++) {
  for (let xx = 0; xx < grid.width; xx++) {
    ...
    distances.set(p.toString(), p.equals(grid.start) ? 0 : Infinity);
  }
}
...
while (true) {
  ...
  if (pos.equals(grid.goal)) {
    console.log(chalk.redBright("Part One: "), distances.get(pos.toString())); // 530 is too high
    exit();
  }
